defmodule Exmud.Session do
  @moduledoc """
  Unified session management for ExMUD players.

  This module provides a central API for managing player sessions across
  all client types (LiveView, TUI, SSH, mobile, etc.). It enables:
  ...
  ### Connecting a Player (from LiveView)
      ...
      def mount(_params, _session, socket) do
        ...
        {:ok, _session_pid} = Exmud.Session.connect(player, :liveview, self())

        # Update room when player enters
        Exmud.Session.update_room(player.id, room.id)
  ...
  ### Admin Operations
      ...
      Exmud.Session.list_online()

      # Send server announcement
      Exmud.Session.broadcast_all({:announcement, "Server restart in 5 minutes"})

      # Kick a player
      Exmud.Session.disconnect(player_id, "Kicked by admin")
  ...
  alias Exmud.Session.{Registry, Server, Supervisor}
  ...
  player connects via any client type (LiveView, SSH, TUI, etc.).
  ...
  ## Examples
      ...
      {:ok, _pid} = Session.connect(player, :tui, self(), %{ip: "127.0.0.1"})
  """
  @spec connect(map(), client_type(), pid(), map()) :: {:ok, pid()}
  def connect(player, client_type, client_pid, meta \\ %{}) do
    {:ok, session_pid} = Supervisor.get_or_start_session(player)
    :ok = Server.register_client(player.id, client_type, client_pid, meta)
    {:ok, session_pid}
  end
