defmodule Exmud.Framework.Resources.FormulaEvaluator do
  @moduledoc """
  ...
  defp parse_factor([{:var, name} | rest], bindings) do
    # Look up variable, default to 0 if not found
    key = String.to_atom(name)
    value = Map.get(bindings, key, 0)
    {value, rest}
  end
  ...
  defp normalize_bindings(bindings) do
    bindings
    |> Enum.map(fn
      {k, v} when is_binary(k) -> {String.to_atom(k), ensure_number(v)}
      {k, v} when is_atom(k) -> {k, ensure_number(v)}
    end)
    |> Map.new()
  end
