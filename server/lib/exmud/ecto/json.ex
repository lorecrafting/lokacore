defmodule Exmud.Ecto.Json do
  ...
  Unlike `Exmud.Ecto.Term` which uses Erlang binary serialization,
  ...
  Code that accesses loaded data must use string keys:
      ...
      # Before (with Exmud.Ecto.Term):
      Map.get(components, :health)

      # After (with Exmud.Ecto.Json):
      Map.get(components, "health")

  ## Example

      schema "entities" do
        field :components, Exmud.Ecto.Json
        field :metadata, Exmud.Ecto.Json
      end
