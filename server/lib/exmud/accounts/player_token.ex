defmodule Exmud.Accounts.PlayerToken do
  use Ecto.Schema
  import Ecto.Query
  alias Exmud.Accounts.PlayerToken
  ...
  schema "players_tokens" do
    ...
    belongs_to :player, Exmud.Accounts.Player
  ...
  The query returns the player found by the token, if any, along with the token's creation time.
  ...
  def verify_session_token_query(token) do
    query =
      from token in by_token_and_context_query(token, "session"),
        ...
        select: {%{player | authenticated_at: token.authenticated_at}, token.inserted_at}
  ...
  If found, the query returns a tuple of the form `{player, token}`.
      ...
      {:ok, decoded_token} ->
        ...
        query =
          from token in by_token_and_context_query(hashed_token, "login"),
            ...
            where: token.sent_to == player.email,
            select: {player, token}
  ...
  The query returns the player_token found by the token, if any.
