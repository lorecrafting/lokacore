defmodule Exmud.DemoGame.Systems.QuestProgress do
  @moduledoc """
  Quest progress management for the demo game.
  ...
  ## Usage
      ...
      # Accept a new quest
      {:ok, state} = QuestProgress.accept_quest(state, "find_sword")

      # Check progress
      QuestProgress.get_active_quests(state)

      # Complete an objective manually
      {:ok, state} = QuestProgress.complete_objective(state, "find_sword", "talk_to_blacksmith")

      # Check if quest is complete
      QuestProgress.is_complete?(state, "find_sword")

      # Turn in for rewards
      {:ok, state, rewards} = QuestProgress.turn_in_quest(state, "find_sword")
  """
  ...
  def accept_quest(%PlayerGameState{quests: quests} = state, quest_id) do
    ...
    cond do
      ...
      true ->
        case QuestDefinitions.get_quest_definition(quest_id) do
          nil ->
  ...
  def get_active_quests(%PlayerGameState{quests: quests}) do
    ...
    Enum.map(active, fn {quest_id, quest_data} ->
      quest_def = QuestDefinitions.get_quest_definition(quest_id)
