defmodule Exmud.DemoGame.Systems.Inventory do
  @moduledoc """
  Inventory management system for the demo game.
  ...
  ## Examples
      ...
      {:ok, %PlayerGameState{inventory: ["sword_01"]}}
  """
  def add_item(%PlayerGameState{} = state, item_id) when is_binary(item_id) do
  ...
  def add_item_unchecked(%PlayerGameState{} = state, item_id) when is_binary(item_id) do
    new_inventory = state.inventory ++ [item_id]
    PlayerGameState.update_state(state, %{inventory: new_inventory})
  end
  ...
  ## Examples
      ...
      {:ok, %PlayerGameState{inventory: []}}
  """
  def remove_item(%PlayerGameState{inventory: inventory} = state, item_id) do
  ...
  ## Examples
      ...
      true
  """
  def has_item?(%PlayerGameState{inventory: inventory}, item_id) do
  ...
  """
  def list_items(%PlayerGameState{inventory: inventory}) do
    inventory
    |> Enum.map(&get_item_details/1)
  ...
  ## Examples
      ...
      {:ok, %PlayerGameState{}, %{healed: 50}}
  """
  def use_item(%PlayerGameState{} = state, item_id) do
    cond do
  ...
  """
  def count(%PlayerGameState{inventory: inventory}), do: length(inventory)

  @doc """
  Returns true if the inventory is empty.
  """
  def empty?(%PlayerGameState{inventory: []}), do: true
  def empty?(%PlayerGameState{}), do: false
  ...
  defp apply_healing(state, item_id, amount) do
    ...
    new_current = min(current_health.current + amount, current_health.max)
    actual_healed = new_current - current_health.current
