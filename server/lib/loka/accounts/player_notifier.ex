defmodule Loka.Accounts.PlayerNotifier do
  import Swoosh.Email
  require Logger
  ...
  defp deliver(recipient, subject, body) do
    from_config = Application.get_env(:loka, :mailer_from, [])
    from_name = Keyword.get(from_config, :name, "Loka")
    from_email = Keyword.get(from_config, :email, "noreply@example.com")
    mailer_config = Application.get_env(:loka, Loka.Mailer, [])
  ...
  def deliver_login_instructions(player, url) do
    Logger.debug(
      "[PlayerNotifier] deliver_login_instructions called [#{LogSanitizer.hash_id(player.email)}]"
    )
