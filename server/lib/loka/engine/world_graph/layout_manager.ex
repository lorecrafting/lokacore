defmodule Loka.Engine.WorldGraph.LayoutManager do
  @moduledoc """
  ...
  alias Loka.Engine.{WorldGraph, Entities, Hooks}
  ...
  defp do_refresh_all do
    # Wrap in try/catch to handle sandbox ownership errors in tests
    try do
      ...
      e in [DBConnection.OwnershipError, DBConnection.ConnectionError, Exqlite.Error] ->
        # In test mode with async tests, we may not have sandbox access
        Logger.debug(
          "[LayoutManager] Skipping refresh_all - DB error (test mode): #{inspect(e.__struct__)}"
        )

        :ok
    catch
      :exit, _ ->
        # Handle exit signals from DB connection issues
        Logger.debug("[LayoutManager] Skipping refresh_all - process exit (test mode)")
        :ok
    end
  end
  ...
  defp do_refresh_from(room_id) do
    ...
    try do
      ...
      e in [DBConnection.OwnershipError, DBConnection.ConnectionError, Exqlite.Error] ->
        # In test mode with async tests, we may not have sandbox access
        Logger.debug(
          "[LayoutManager] Skipping refresh - DB error (test mode): #{inspect(e.__struct__)}"
        )
      ...
      :exit, _ ->
        # Handle exit signals from DB connection issues
        Logger.debug("[LayoutManager] Skipping refresh - process exit (test mode)")
        :ok
    end
  end
