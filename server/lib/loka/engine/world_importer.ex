defmodule Loka.Engine.WorldImporter do
  @moduledoc """
  ...
  def import_all(input_dir, opts \\ []) do
    ...
    if clear_existing do
      ...
      LokaWeb.Channels.RoomHelpers.clear_minimap_cache()
    end
  ...
  """
  # sobelow_skip ["Traversal.FileModule"] - filepath from admin import directory selection
  ...
  def create_entity_from_yaml(data, expected_type) do
    ...
    entity = %Entity{
      id: UUID.uuid4(),
      type: type,
      key: data["key"] || generate_key(data["short_desc"]),
      short_desc: data["short_desc"] || "Unnamed",
  ...
  @valid_types [:room, :npc, :item, :exit]

  # Parse type from string or use default
  defp parse_type(nil, default), do: default

  defp parse_type(type_str, default) when is_binary(type_str) do
    case MapHelpers.safe_to_existing_atom(type_str) do
      nil ->
        Logger.warning("Unknown entity type in import: #{inspect(type_str)}, using default")
        default

      atom when atom in @valid_types ->
        atom

      _other ->
        Logger.warning("Invalid entity type in import: #{inspect(type_str)}, using default")
        default
    end
  end

  defp parse_type(type, _default) when is_atom(type) and type in @valid_types, do: type
  defp parse_type(_type, default), do: default
  ...
  # Recursively atomize attribute keys (safely, using existing atoms only)
  defp atomize_keys(nil), do: %{}
  defp atomize_keys(map) when is_map(map), do: MapHelpers.deep_atomize_keys(map)
