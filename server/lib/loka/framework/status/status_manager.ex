defmodule Loka.Framework.Status.StatusManager do
  @moduledoc """
  ...
  Uses Loka.Mechanics for damage, healing, and resource drain calculations
  to ensure consistency with combat and other game systems.
  ...
  defp process_effect(effect, active) do
    ...
    case action do
      :damage ->
        ...
        mock_health = ResourcePool.new(current: 1000, max: 1000)
        {:ok, _pool, damage_result} = Damage.apply_to(mock_health, amount)

        Map.merge(base_result, %{
          amount: amount,
          damage_type: damage_type,
          mechanics_result: damage_result
        })
      ...
      :heal ->
        ...
        {:ok, _pool, heal_result} = Heal.apply_to(mock_health, amount)

        Map.merge(base_result, %{
          amount: amount,
          mechanics_result: heal_result
        })
  ...
  defp normalize_stat(stat) when is_binary(stat) do
    Loka.Utils.MapHelpers.normalize_atom(stat, :unknown)
  end
