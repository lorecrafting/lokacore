defmodule Loka.Framework.Conditions.Evaluator do
  ...
  ## Examples
      ...
      iex> Evaluator.evaluate({:flag, "spoke_to_elder"}, game_state)
      true

      iex> Evaluator.evaluate(%{"quest_completed" => "main_quest"}, game_state)
      false

      iex> Evaluator.evaluate_all([{:level_gte, 5}, {:flag, "unlocked"}], game_state)
      true
  """
  ...
  """
  @spec evaluate(term(), GameState.t()) :: boolean()
  def evaluate(condition, %GameState{} = state) do
    condition
    |> normalize()
    |> do_evaluate(state)
  end
  ...
  """
  @spec evaluate_all(list(), GameState.t()) :: boolean()
  def evaluate_all(conditions, %GameState{} = state) when is_list(conditions) do
    Enum.all?(conditions, &evaluate(&1, state))
  end

  def evaluate_all(nil, _state), do: true
  def evaluate_all([], _state), do: true
  ...
  """
  @spec evaluate_any(list(), GameState.t()) :: boolean()
  def evaluate_any(conditions, %GameState{} = state) when is_list(conditions) do
    Enum.any?(conditions, &evaluate(&1, state))
  end

  def evaluate_any(nil, _state), do: false
  def evaluate_any([], _state), do: false
  ...
  """
  @spec evaluate_show_if(map() | nil, GameState.t()) :: boolean()
  def evaluate_show_if(nil, _state), do: true

  def evaluate_show_if(condition, %GameState{} = state) when is_map(condition) do
    # Each key in the map is an independent condition
    Enum.all?(condition, fn {key, value} ->
      evaluate({key, value}, state)
    end)
  end
  ...
  defp do_evaluate(:default, _state), do: true

  # Flag conditions
  defp do_evaluate({:flag, flag_name}, %GameState{flags: flags}) do
    flags = flags || %{}
    truthy?(get_flexible(flags, flag_name, nil))
  end

  # Quest completion conditions
  defp do_evaluate({:quest_completed, quest_id}, %GameState{quests: quests}) do
    completed = StateHelper.get_completed(quests)
    quest_id in completed
  end

  defp do_evaluate({:quest_not_completed, quest_id}, %GameState{quests: quests}) do
    completed = StateHelper.get_completed(quests)
    quest_id not in completed
  end

  # Quest active conditions
  defp do_evaluate({:quest_active, quest_id}, %GameState{quests: quests}) do
    active_quests = StateHelper.get_active(quests)
    Map.has_key?(active_quests, quest_id)
  end

  defp do_evaluate({:quest_not_active, quest_id}, %GameState{quests: quests}) do
    active_quests = StateHelper.get_active(quests)
    not Map.has_key?(active_quests, quest_id)
  end

  # Level condition
  defp do_evaluate({:level_gte, level}, %GameState{stats: stats}) do
    stats = stats || %{}
  ...
  defp do_evaluate({:item_has, item_id}, %GameState{inventory: inventory}) do
    inventory = inventory || []
    has_item?(inventory, item_id)
  end

  defp do_evaluate({:has_item, item_id}, state) do
    do_evaluate({:item_has, item_id}, state)
  end

  defp do_evaluate({:not_has_item, item_id}, state) do
    not do_evaluate({:item_has, item_id}, state)
  end

  # Stat condition
  defp do_evaluate({:stat_gte, stat, value}, %GameState{stats: stats}) do
    stats = stats || %{}
  ...
  defp do_evaluate({:faction_gte, faction_id, value}, %GameState{flags: flags}) do
    flags = flags || %{}
  ...
  defp do_evaluate(unknown, _state) do
