defmodule Loka.Framework.Gathering.GatheringNode do
  @moduledoc """
  Gathering node definition struct and parsing using the Roll primitive.
  ...
  Uses the Roll primitive for:
  - Yield chance rolls (percentage-based success checks)
  - Quantity ranges (random amounts within min/max)
  ...
  Handles both fixed quantities and random ranges using the Roll primitive.
  """
  ...
  Uses the Roll primitive for percentage-based success checks.
  """
  def roll_yields(%__MODULE__{yields: yields}) do
    yields
    |> Enum.filter(fn %{chance: chance} ->
      # Convert decimal chance (0.6) to percentage (60)
  ...
  @doc """
  Rolls for yields with detailed audit information.

  Returns `{yields, audit}` where audit contains the roll results.
  """
  def roll_yields_with_audit(%__MODULE__{yields: yields}) do
    {results, audits} =
      yields
      |> Enum.map(fn %{chance: chance, item: item, quantity: quantity} ->
        target_percent = round(chance * 100)
        {:ok, result, audit} = Roll.check(target_percent)

        yield_result =
          if result.success do
            qty = calculate_quantity(quantity)
            {:ok, %{item: item, quantity: qty}}
          else
            {:failed, item}
          end

        {yield_result,
         %{item: item, chance: chance, roll: result.roll, success: result.success, audit: audit}}
      end)
      |> Enum.unzip()
    ...
    successful_yields =
      ...
      |> Enum.map(fn {:ok, yield} -> yield end)

    {successful_yields, audits}
  end
end
