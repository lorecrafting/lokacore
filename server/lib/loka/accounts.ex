defmodule Loka.Accounts do
  ...
  Handles player authentication, sessions, and admin management.
  This is separate from game logic (Engine/Framework).
  """
  ...
  @doc """
  Gets or creates a guest player by device ID.

  If a player with the given device_id exists, returns it.
  Otherwise, creates a new guest player with the device_id.
  """
  def get_or_create_guest(device_id, name \\ nil) do
    case Repo.get_by(Player, device_id: device_id) do
      nil ->
        %Player{}
        |> Player.guest_changeset(%{device_id: device_id, name: name})
        |> Repo.insert()

      player ->
        # If a name is provided and the player doesn't have one, update it
        if name && name != "" && (is_nil(player.name) || player.name == "") do
  ...
  def update_player_name(player, name) do
    player
    |> Player.name_changeset(%{name: name})
    |> Repo.update()
  end
  ...
  See `Loka.Accounts.Player.email_changeset/3` for a list of supported options.
  ...
  See `Loka.Accounts.Player.password_changeset/3` for a list of supported options.
  ...
  defp update_player_and_delete_all_tokens(changeset) do
    Repo.transact(fn ->
      with {:ok, player} <- Repo.update(changeset) do
        # Query for tokens first so we can return them for verification
        tokens_to_expire =
          Repo.all(
            from(t in PlayerToken, where: t.player_id == ^player.id, select: %{token: t.token})
          )

        # Delete all tokens for this player
        Repo.delete_all(from(t in PlayerToken, where: t.player_id == ^player.id))
