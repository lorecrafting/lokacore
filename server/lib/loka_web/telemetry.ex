defmodule LokaWeb.Telemetry do
  @moduledoc """
  Telemetry and metrics collection for Loka.
  ...
  def metrics do
    [
      ...
      last_value("loka.game.active_players.count",
        description: "Number of currently active players"
      ),
      last_value("loka.game.active_entities.count",
        description: "Number of active entity processes"
      ),
      last_value("loka.game.rooms_loaded.count",
        description: "Number of rooms currently loaded in memory"
      ),
      counter("loka.game.commands.total",
        description: "Total commands executed",
        tags: [:command]
      ),
      counter("loka.game.combat.started.total",
        description: "Total combat encounters started"
      ),
      counter("loka.game.login.total",
        description: "Total player logins"
      ),
      counter("loka.auth.rate_limited.total",
  ...
  def measure_game_stats do
    ...
    :telemetry.execute(
      [:loka, :game, :active_players],
      %{count: active_players},
      %{}
    )

    :telemetry.execute(
      [:loka, :game, :active_entities],
  ...
  defp count_active_game_sessions do
    # Count active GameChannel connections via PubSub
    try do
      # Count unique players subscribed to game channels
      Phoenix.PubSub.node_name(Loka.PubSub)
      # Return 0 for now - accurate counting requires tracking in GameChannel
      0
    rescue
      _ -> 0
    end
