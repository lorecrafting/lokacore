...
The session system provides unified client messaging for ExMUD, enabling multi-client support, admin visibility, and graceful disconnect handling.
...
1. **Elixir Registry** (`Exmud.Session.PlayerRegistry`)
   - Maps `player_id` → `session_pid`
   - O(1) lookup
   - Automatic cleanup on process termination

2. **ETS Table** (`:exmud_session_room_index`)
...
| `:tui` | Unix Socket + JSON-RPC | 🔧 Available |
...
| Feature | Evennia Portal | ExMUD Session |
|---------|----------------|---------------|
| Unified session | ✅ | ✅ |
| Multi-client | ✅ | ✅ |
| Reconnect grace | ✅ | ✅ (30s) |
...
| File | Purpose |
|------|---------|
| `lib/exmud/session.ex` | Public API |
| `lib/exmud/session/registry.ex` | Player & room lookups |
| `lib/exmud/session/server.ex` | Per-player GenServer |
| `lib/exmud/session/supervisor.ex` | DynamicSupervisor |
