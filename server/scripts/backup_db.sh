#!/bin/bash
# Database backup script for Loka on Fly.io
...
BACKUP_NAME="loka_backup_${TIMESTAMP}.db"

echo "=== Loka Database Backup ==="
...
cd "$BACKUP_DIR"
ls -t loka_backup_*.db.gz 2>/dev/null | tail -n +$((MAX_BACKUPS + 1)) | xargs -r rm -f

# List current backups
echo ""
echo "Current backups:"
ls -lh "$BACKUP_DIR"/loka_backup_*.db.gz 2>/dev/null || echo "No backups found"
