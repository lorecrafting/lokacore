defmodule Exmud.Framework.Status.StatusManagerTest do
  use ExUnit.Case, async: false

  alias Exmud.Framework.Status.{StatusManager, StatusRegistry, StatusEffect}
  ...
  setup %{temp_dir: temp_dir} do
    ...
    on_exit(fn ->
      ...
      try do
        :ets.delete_all_objects(:exmud_active_statuses)
      rescue
        _ -> :ok
      end
    end)
  ...
  describe "apply_status/4" do
    ...
    test "refreshes duration for non-stackable status" do
      ...
      :ets.insert(:exmud_active_statuses, {entity_id, modified})
  ...
  describe "tick/2" do
    test "processes status effects for trigger" do
      ...
      # BUG: damage_type should be :poison but is :physical (default) due to same issue
