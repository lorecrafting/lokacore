defmodule Exmud.Framework.CraftingTest do
  use Exmud.DataCase

  alias Exmud.Framework.Crafting
  alias Exmud.Framework.Crafting.{Recipe, CraftingRegistry}
  ...
  setup do
    # Start CraftingRegistry for each test
    {:ok, registry} = start_supervised({CraftingRegistry, [load_on_start: false]})
  ...
  describe "craft/3" do
    test "successfully crafts item with no failure chance", %{simple_recipe: simple_recipe} do
      ...
      assert {:ok, result} = Crafting.craft(state, simple_recipe.key)
    ...
    test "handles failure with failure_output", %{skilled_recipe: skilled_recipe} do
      ...
      results =
        Enum.map(1..10, fn _ ->
          {:ok, result} = Crafting.craft(state, skilled_recipe.key, room: forge)
          result.success
        end)
    ...
    test "applies station bonus to reduce failure chance", %{skilled_recipe: skilled_recipe} do
      ...
      {:ok, result} = Crafting.craft(state, skilled_recipe.key, room: forge_with_bonus)

      # With bonus, failure chance is 0.2 - 0.2 = 0.0, so always succeeds
      assert result.success == true
    end

    test "includes consumed ingredients in result" do
      player = player_fixture()
      ...
      {:ok, result} = Crafting.craft(state, "recipe_simple_item")
  ...
  describe "edge cases" do
    ...
    test "station bonus cannot reduce failure chance below 0" do
      ...
      {:ok, result} = Crafting.craft(state, "recipe_low_fail", room: forge)
  ...
  describe "full crafting workflow integration" do
    test "complete crafting workflow from check to consume" do
      player = player_fixture()
      ...
      # Step 3: Execute crafting
      assert {:ok, craft_result} = Crafting.craft(state, recipe_key)
      ...
      assert {:ok, final_state} = Crafting.consume_ingredients(state, recipe_key)
