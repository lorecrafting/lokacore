defmodule Exmud.Engine.EntityServerTest do
  use Exmud.DataCase, async: false

  alias Exmud.Engine.EntityServer
  ...
  describe "get_entity/1" do
    test "returns the current entity state" do
      schema = entity_fixture(%{name: "Test Entity"})
  ...
  describe "update/2" do
    test "updates entity with function" do
      ...
      assert EntityServer.get_entity(pid).name == "Updated"
  ...
  describe "auto-save timer" do
    test "automatically saves dirty state after interval" do
      ...
      # Verify in database
      reloaded = Entities.get_entity(schema.id)
      assert reloaded.name == "Auto-saved"
  ...
  describe "idle timeout" do
    ...
    test "saves state before stopping due to idle" do
      ...
      reloaded = Entities.get_entity(schema.id)
      assert reloaded.name == "Before Idle Stop"
    end
  end
  ...
  describe "stop/1" do
    ...
    test "saves dirty state before stopping" do
      ...
      # Verify state was saved
      reloaded = Entities.get_entity(schema.id)
      assert reloaded.name == "Stopped State"
    end
  end
