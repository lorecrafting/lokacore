defmodule ExmudWeb.ConnCase do
  ...
  by setting `use ExmudWeb.ConnCase, async: true`, although
  ...
  using do
    quote do
      ...
      import ExmudWeb.ConnCase
    end
  end

  setup tags do
    Exmud.DataCase.setup_sandbox(tags)
  ...
  def register_and_log_in_player(%{conn: conn} = context) do
    player = Exmud.AccountsFixtures.player_fixture()
  ...
  def log_in_player(conn, player, opts \\ []) do
    token = Exmud.Accounts.generate_player_session_token(player)
  ...
  defp maybe_set_token_authenticated_at(token, authenticated_at) do
    Exmud.AccountsFixtures.override_token_authenticated_at(token, authenticated_at)
  end
end
