defmodule Loka.Framework.Resources.ResourcePoolTest do
    ...
    pool_name =
      case Process.whereis(ResourcePool) do
        nil ->
          ...
          {:ok, _pid} =
            start_supervised({ResourcePool, name: name}, id: name)

          name

        _pid ->
          # Use the global pool and clear it for test isolation
          ResourcePool.clear_all()
          ResourcePool
      end
  ...
  describe "init_pools/2" do
    ...
    test "initializes resources that don't start full at min_value", %{pool: pool} do
      ...
      # With starts_full: false, energy starts at min_value (0) not max
      assert pools["energy"].current == 0
  ...
  describe "tick_regen/2" do
    test "regenerates resources based on condition", %{pool: pool} do
      ...
      # Should have regenerated (at least 2 per tick, possibly more if global tick fired)
      health = ResourcePool.current(entity_id, "health")
      assert health >= 52, "Expected health >= 52 after regen, got #{health}"
      assert health <= 100, "Health should not exceed max"
    end
    ...
    test "regenerates multiple resources with :always condition", %{pool: pool} do
      ...
      # Get energy immediately before triggering regen
      energy_before = ResourcePool.current(entity_id, "energy")

      # Energy regens always (rate: 1)
      GenServer.cast(pool, {:tick_regen, entity_id, %{}})
      Process.sleep(50)

      energy_after = ResourcePool.current(entity_id, "energy")
      # Energy should have regenerated by at least 1 (may be more due to global tick timer)
  ...
  describe "tick_all_regen/1" do
    test "regenerates all entities", %{pool: pool} do
      ...
      # Use range assertions to handle timing variations from global tick timer
      GenServer.cast(pool, {:tick_all_regen, %{in_combat: false}})
      Process.sleep(50)

      health1 = ResourcePool.current(entity1, "health")
      health2 = ResourcePool.current(entity2, "health")
      assert health1 >= 52 and health1 <= 100, "Expected health1 >= 52, got #{health1}"
      assert health2 >= 72 and health2 <= 100, "Expected health2 >= 72, got #{health2}"
    end
  end
