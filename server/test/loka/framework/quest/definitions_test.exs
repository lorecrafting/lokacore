defmodule Loka.Framework.Quest.DefinitionsTest do
  use Loka.DataCase

  alias Loka.Framework.Quest.Definitions
  alias Loka.Framework.Quest.Definitions.{Quest, Objective}
  ...
  defp create_quest_entity(attrs \\ %{}) do
    {:ok, entity} =
      Entities.create_entity(%{
        ...
        extra_desc: attrs[:description] || "A test quest for testing",
        type: "item",
        components:
          attrs[:components] ||
            %{
  ...
  describe "objective_types/0" do
    test "returns all valid objective types" do
      ...
      # Must include the 4 core types
      assert :talk in types
      assert :kill in types
      assert :get_item in types
      assert :go_to in types
      # At least 4 types (may be more if custom handlers are registered)
