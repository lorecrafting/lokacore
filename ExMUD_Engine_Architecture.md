# ExMUD: Elixir MUD Engine Framework
...
ExMUD is an **engine framework** (not a game) built on Elixir/Phoenix/LiveView, designed to provide the primitives, abstractions, and skeleton that game creators—and LLMs assisting them—can build upon. Similar to how Evennia serves the Python ecosystem, ExMUD leverages Elixir's unique strengths in concurrency, fault-tolerance, and real-time communication to create a modern MUD development platform.
...
### 1.2 What ExMUD Provides (Engine Core)
...
Unlike Evennia's typeclass system (which uses Python class inheritance), ExMUD uses a composition-based **Entity-Component-Behavior** model that maps naturally to Elixir's functional paradigm.

```elixir
defmodule ExMUD.Entity do
  @moduledoc """
...
defmodule ExMUD.Behaviors.DefaultObject do
  @behaviour ExMUD.Behavior
...
defmodule ExMUD.Behaviors.NPC do
  @behaviour ExMUD.Behavior
  ...
  def handle_event(entity, %Event{type: :tick}, _context) do
    ...
    case entity.scripts[:on_tick] do
      nil -> {:ok, entity}
      script -> 
        ExMUD.Scripting.execute(script, entity)
    end
  end
  ...
  def handle_event(entity, %Event{type: :player_entered, payload: player}, context) do
    ...
    case entity.scripts[:on_player_enter] do
      nil -> {:ok, entity}
      script ->
        ExMUD.Scripting.execute(script, entity, %{player: player})
    end
  end
end
```
...
```elixir
defmodule ExMUD.EntityLifecycle do
  @moduledoc """
...
defmodule ExMUD.EntityServer do
  use GenServer
  ...
  def init(entity_id) do
    ...
    entity = ExMUD.Repo.get_entity(entity_id)
    entity = ExMUD.EntityLifecycle.at_entity_init(entity)
  ...
  def handle_call({:process_event, event, context}, _from, state) do
    case ExMUD.EventProcessor.process(state.entity, event, context) do
      {:ok, updated_entity, emitted_events} ->
        # Broadcast emitted events
        Enum.each(emitted_events, &ExMUD.EventBus.broadcast/1)
  ...
  def handle_info(:auto_save, %{dirty?: true} = state) do
    ExMUD.Repo.save_entity(state.entity)
    Process.send_after(self(), :auto_save, :timer.minutes(5))
...
```elixir
defmodule ExMUD.World.TickServer do
  @moduledoc """
...
defmodule ExMUD.Commands.Look do
  ...
  def parse(args, context) do
    case ExMUD.Search.find_target(args, context) do
...
```elixir
defmodule ExMUD.CommandPipeline do
  @moduledoc """
  ...
  def process(raw_input, session) do
    with {:ok, {command_module, args}} <- route(raw_input, session),
      ...
      Enum.each(events, &ExMUD.EventBus.emit/1)
      :ok
    else
      {:error, message} ->
        ExMUD.Session.send_message(session, message)
        :ok
    end
  end
  ...
  defp route(raw_input, session) do
    ...
    case ExMUD.CommandRegistry.find(cmd, session.command_sets) do
...
defmodule ExMUD.CommandSet do
  ...
  defmodule Combat do
    def commands do
      [
        ExMUD.Commands.Attack,
        ExMUD.Commands.Defend,
        ExMUD.Commands.Flee,
        ExMUD.Commands.UseSkill,
        ExMUD.Commands.CastSpell,
      ]
    end
  end
  ...
  defmodule Builder do
    def commands do
      [
        ExMUD.Commands.Admin.Create,
        ExMUD.Commands.Admin.Edit,
        ExMUD.Commands.Admin.Delete,
        ExMUD.Commands.Admin.Teleport,
        ExMUD.Commands.Admin.Spawn,
      ]
    end
  end
end
...
defmodule ExMUD.EventBus do
  ...
  def emit(%Event{} = event) do
    ...
    if event.target do
      ExMUD.EntityServer.process_event(event.target, event)
    end
  end
...
```elixir
defmodule ExMUD.Scripting.Security do
  @moduledoc """
...
defmodule ExMUDWeb.GameLive do
  ...
  def mount(%{"game_id" => game_id}, session, socket) do
    if connected?(socket) do
      ...
      {:ok, session_pid} = ExMUD.Session.join(session.player_id)
  ...
  def handle_event("command", %{"input" => input}, socket) do
    ExMUD.Session.process_command(socket.assigns.session_pid, input)
    {:noreply, socket}
  end
  
  def handle_event("move", %{"direction" => dir}, socket) do
    ExMUD.Session.process_command(socket.assigns.session_pid, dir)
    {:noreply, socket}
  end
...
```elixir
defmodule ExMUDWeb.Components.RoomView do
...
```javascript
// exmud_crypto.js - Runs in browser
class ExMUDCrypto {
  constructor() {
...
```elixir
defmodule ExMUD.Messaging.E2E do
  @moduledoc """
  ...
  def send_encrypted_tell(sender_id, recipient_id, encrypted_payload) do
    ...
    ExMUD.Repo.insert!(message)
    
    # If recipient online, deliver immediately
    case ExMUD.Session.find(recipient_id) do
      {:ok, session_pid} ->
        ExMUD.Session.deliver_encrypted_message(session_pid, message)
      :not_found ->
...
```elixir
# Core entity storage
defmodule ExMUD.Schema.Entity do
  use Ecto.Schema
...
```elixir
defmodule ExMUDWeb.Admin.WorldBuilderLive do
  use ExMUDWeb, :live_view
...
```elixir
defmodule ExMUDWeb.Admin.ScriptEditorLive do
  use ExMUDWeb, :live_view
    ...
    <div class="script-editor">
      <div class="editor-header">
        <select phx-change="select_hook">
          <%= for hook <- ExMUD.EventHooks.available_hooks() do %>
  ...
  def handle_event("validate", _params, socket) do
    result = ExMUD.Scripting.Security.validate_script(socket.assigns.source)
    {:noreply, assign(socket, validation_result: result)}
  end
  
  def handle_event("test", _params, socket) do
    # Run script in isolated test environment
    output = ExMUD.Scripting.test_run(
...
```elixir
defmodule ExMUD.Config.GameRules do
  @moduledoc """
...
ExMUD supports multiple client types connecting to the same game engine:
...
```elixir
defmodule ExMUDWeb.GameChannel do
  use Phoenix.Channel
  ...
  def join("game:" <> game_id, %{"token" => token}, socket) do
    case ExMUD.Auth.verify_token(token) do
      {:ok, player_id} ->
        # Start or resume player session
        {:ok, session} = ExMUD.Session.join(player_id, game_id)
        
        # Subscribe to player-specific events
        Phoenix.PubSub.subscribe(ExMUD.PubSub, "player:#{player_id}")
        Phoenix.PubSub.subscribe(ExMUD.PubSub, "game:#{game_id}")
  ...
  defp detect_client_type(socket) do
    ...
    case get_connect_info(socket, :user_agent) do
      ua when is_binary(ua) ->
        cond do
          String.contains?(ua, "ExMUD-iOS") -> :ios
          String.contains?(ua, "ExMUD-Android") -> :android
          String.contains?(ua, "ExMUD-Terminal") -> :terminal
          true -> :web
        end
      _ -> :unknown
    end
  ...
  def handle_info(:after_join, socket) do
    state = ExMUD.Session.get_state(socket.assigns.session)
  ...
  defp format_game_state(state, :terminal) do
    ...
    %{
      room_text: ExMUD.Formatter.to_plain_text(state.room),
  ...
  def handle_in("command", %{"input" => input}, socket) do
    result = ExMUD.Session.process_command(
  ...
  def handle_in("move", %{"direction" => dir}, socket) do
    ExMUD.Session.process_command(socket.assigns.session, dir)
    {:noreply, socket}
  end
  
  def handle_in("look", params, socket) do
    target = Map.get(params, "target", "")
    ExMUD.Session.process_command(socket.assigns.session, "look #{target}")
    {:noreply, socket}
  end
  
  def handle_in("get", %{"item" => item}, socket) do
    ExMUD.Session.process_command(socket.assigns.session, "get #{item}")
    {:noreply, socket}
  end
  
  def handle_in("attack", %{"target" => target}, socket) do
    ExMUD.Session.process_command(socket.assigns.session, "attack #{target}")
    {:noreply, socket}
  end
  
  def handle_in("use_skill", %{"skill" => skill, "target" => target}, socket) do
    ExMUD.Session.process_command(socket.assigns.session, "use #{skill} #{target}")
    {:noreply, socket}
  end
  
  def handle_in("cast", %{"spell" => spell, "target" => target}, socket) do
    ExMUD.Session.process_command(socket.assigns.session, "cast #{spell} #{target}")
    {:noreply, socket}
  end
  ...
  def handle_in("encrypted_tell", payload, socket) do
    ...
    ExMUD.Messaging.E2E.send_encrypted_tell(
  ...
  def handle_in("get_key_bundle", %{"player_id" => target_id}, socket) do
    case ExMUD.Messaging.E2E.get_key_bundle(target_id) do
      {:ok, bundle} ->
        {:reply, {:ok, %{
  ...
  defp format_event(event, :terminal) do
    ...
    %{
      text: ExMUD.Formatter.to_ansi(event),
      raw: event.payload
    }
  end
...
```elixir
defmodule ExMUD.Telnet.Server do
  @moduledoc """
  ...
  defp accept_loop(listen_socket) do
    case :gen_tcp.accept(listen_socket) do
      {:ok, client_socket} ->
        {:ok, pid} = ExMUD.Telnet.Session.start_link(client_socket)
...
defmodule ExMUD.Telnet.Session do
  @moduledoc """
  ...
  def handle_info({:tcp, socket, data}, %{state: {:password, username}} = state) do
    ...
    case ExMUD.Auth.authenticate(username, password) do
      {:ok, player_id, token} ->
        # Join the game
        {:ok, game_session} = ExMUD.Session.join(player_id, "default")
        
        # Subscribe to events
        Phoenix.PubSub.subscribe(ExMUD.PubSub, "player:#{player_id}")
        
        # Send initial room description
        game_state = ExMUD.Session.get_state(game_session)
  ...
  def handle_info({:tcp, socket, data}, %{state: :playing} = state) do
    ...
    unless input == "" do
      ExMUD.Session.process_command(state.game_session, input)
    end
  ...
  def handle_info({:game_event, event}, state) do
    ...
    game_state = ExMUD.Session.get_state(state.game_session)
  ...
  def handle_info({:encrypted_message, message}, state) do
    ...
    sender = ExMUD.Entities.get_name(message.sender_id)
  ...
  defp welcome_banner do
    ...
    ╚══════╝╚═╝  ╚═╝╚═╝     ╚═╝ ╚═════╝ ╚═════╝ 
    \e[0m
    Welcome to ExMUD - A Modern Text Adventure
...
defmodule ExMUD.Command.Parser do
  ...
  def parse(input, command_sets) do
    ...
    case input do
      "'" <> rest -> {:ok, {ExMUD.Commands.Say, String.trim(rest)}}
      "\"" <> rest -> {:ok, {ExMUD.Commands.Say, String.trim(rest)}}
      ":" <> rest -> {:ok, {ExMUD.Commands.Emote, String.trim(rest)}}
      ";" <> rest -> {:ok, {ExMUD.Commands.SemiEmote, String.trim(rest)}}
  ...
  defp find_command(cmd, args, command_sets) do
    ...
    case direction_shortcut(cmd_lower) do
      {:ok, direction} ->
        {:ok, {ExMUD.Commands.Move, direction}}
...
```elixir
defmodule ExMUD.Economy.Wallet do
  @moduledoc """
  ...
  defp get_or_create_wallet(player_id) do
    case ExMUD.Repo.get_by(__MODULE__, player_id: player_id) do
      nil ->
        %__MODULE__{}
...
```elixir
defmodule ExMUD.Store.Products do
  @moduledoc """
  ...
  @products %{
    ...
    "gems_100" => %{
      ...
      price_usd: 0.99,
      type: :consumable,
      apple_product_id: "com.exmud.gems100",
    ...
    "gems_500" => %{
      ...
      type: :consumable,
      apple_product_id: "com.exmud.gems500",
    ...
    "gems_1200" => %{
      ...
      type: :consumable,
      apple_product_id: "com.exmud.gems1200",
    ...
    "gems_2500" => %{
      ...
      type: :consumable,
      apple_product_id: "com.exmud.gems2500",
    ...
    "gems_6500" => %{
      ...
      type: :consumable,
      apple_product_id: "com.exmud.gems6500",
    ...
    "sub_basic_monthly" => %{
      ...
      type: :subscription,
      apple_product_id: "com.exmud.sub.basic.monthly",
    ...
    "sub_premium_monthly" => %{
      ...
      type: :subscription,
      apple_product_id: "com.exmud.sub.premium.monthly",
...
```elixir
defmodule ExMUD.Store.Apple do
  @moduledoc """
  ...
  defp get_private_key do
    Application.get_env(:exmud, :apple_private_key)
  end
  
  defp app_store_issuer_id do
    Application.get_env(:exmud, :apple_issuer_id)
  end
  
  defp bundle_id do
    Application.get_env(:exmud, :apple_bundle_id)
  end
end
```
...
```elixir
defmodule ExMUD.Store.Google do
  @moduledoc """
  ...
  defp package_name do
    Application.get_env(:exmud, :google_package_name)
  end
...
```elixir
defmodule ExMUD.Store.Stripe do
  @moduledoc """
  ...
  def create_checkout_session(player_id, product_id, success_url, cancel_url) do
    product = ExMUD.Store.Products.get_product(product_id)
  ...
  def handle_webhook(payload, signature) do
    webhook_secret = Application.get_env(:exmud, :stripe_webhook_secret)
  ...
  defp process_event(%{type: "checkout.session.completed"} = event) do
    ...
    case product.type do
      :consumable ->
        # Credit gems
        total_gems = ExMUD.Store.Products.total_gems(product)
        ExMUD.Economy.Wallet.credit_gems(
          player_id,
          total_gems,
      ...
      :subscription ->
        ...
        ExMUD.Economy.Wallet.activate_subscription(
          player_id,
          product.tier,
  ...
  defp process_event(%{type: "customer.subscription.updated"} = event) do
    ...
    if subscription.status == "active" do
      ExMUD.Economy.Wallet.update_subscription_expiry(
        player_id,
  ...
  defp process_event(%{type: "customer.subscription.deleted"} = event) do
    ...
    ExMUD.Economy.Wallet.cancel_subscription(player_id)
    :ok
  end
...
```elixir
defmodule ExMUDWeb.API.StoreController do
  ...
  def products(conn, _params) do
    products = ExMUD.Store.Products.list_products()
  ...
  def verify_apple(conn, %{"transaction_id" => transaction_id}) do
    ...
    case ExMUD.Store.Apple.verify_transaction(transaction_id) do
      {:ok, transaction} ->
        product = ExMUD.Store.Products.get_product(transaction.product_id)
        
        case product.type do
          :consumable ->
            total_gems = ExMUD.Store.Products.total_gems(product)
            {:ok, wallet} = ExMUD.Economy.Wallet.credit_gems(
              player_id,
              total_gems,
          ...
          :subscription ->
            {:ok, _wallet} = ExMUD.Economy.Wallet.activate_subscription(
              player_id,
              product.tier,
  ...
  def verify_google(conn, %{"product_id" => product_id, "purchase_token" => token}) do
    ...
    verify_fn = case product.type do
      :subscription -> &ExMUD.Store.Google.verify_subscription/2
      _ -> &ExMUD.Store.Google.verify_purchase/2
    end
    
    case verify_fn.(product.google_product_id, token) do
      {:ok, purchase} ->
        # Acknowledge the purchase
        :ok = ExMUD.Store.Google.acknowledge_purchase(product.google_product_id, token)
  ...
  def create_checkout(conn, %{"product_id" => product_id}) do
    ...
    success_url = ExMUDWeb.Router.Helpers.store_url(conn, :success)
    cancel_url = ExMUDWeb.Router.Helpers.store_url(conn, :cancel)
    
    case ExMUD.Store.Stripe.create_checkout_session(player_id, product_id, success_url, cancel_url) do
      {:ok, session} ->
  ...
  def stripe_webhook(conn, _params) do
    ...
    case ExMUD.Store.Stripe.handle_webhook(payload, signature) do
      :ok ->
...
```elixir
defmodule ExMUD.Store.Transaction do
  @moduledoc """
...
defmodule ExMUDWeb.Router do
  ...
  pipeline :browser do
    ...
    plug :put_root_layout, {ExMUDWeb.Layouts, :root}
  ...
  pipeline :player do
    plug :browser
    plug ExMUDWeb.Auth.PlayerAuthPlug
    plug :put_root_layout, {ExMUDWeb.Layouts, :game}
  end
  
  scope "/play", ExMUDWeb do
    pipe_through :player
    
    live_session :game, on_mount: [ExMUDWeb.Auth.LiveAuth] do
  ...
  pipeline :creator do
    plug :browser
    plug ExMUDWeb.Auth.CreatorAuthPlug
    plug :put_root_layout, {ExMUDWeb.Layouts, :creator}
  end
  
  scope "/create", ExMUDWeb.Creator do
    pipe_through :creator
    
    live_session :creator, on_mount: [ExMUDWeb.Auth.CreatorLiveAuth] do
  ...
  pipeline :admin do
    plug :browser
    plug ExMUDWeb.Auth.AdminAuthPlug
    plug :put_root_layout, {ExMUDWeb.Layouts, :admin}
  end
  
  scope "/admin", ExMUDWeb.Admin do
    pipe_through :admin
    
    live_session :admin, on_mount: [ExMUDWeb.Auth.AdminLiveAuth] do
      live "/", DashboardLive, :index
  ...
  scope "/webhooks", ExMUDWeb.API do
    pipe_through :public_api
...
```elixir
defmodule ExMUDWeb.GameLive.Play do
  use ExMUDWeb, :live_view
  ...
  def mount(%{"game_id" => game_id}, session, socket) do
    if connected?(socket) do
      ...
      {:ok, game_session} = ExMUD.Session.join(player_id, game_id)
  ...
  def handle_event("submit_command", %{"command" => command}, socket) do
    ...
    if command != "" do
      ExMUD.Session.process_command(socket.assigns.game_session, command)
  ...
  def handle_event("command", %{"cmd" => cmd}, socket) do
    ExMUD.Session.process_command(socket.assigns.game_session, cmd)
    {:noreply, socket}
  end
  
  @impl true
  def handle_event("move", %{"direction" => direction}, socket) do
    ExMUD.Session.process_command(socket.assigns.game_session, direction)
    {:noreply, socket}
  end
  
  @impl true
  def handle_event("look", %{"target" => target}, socket) do
    ExMUD.Session.process_command(socket.assigns.game_session, "look #{target}")
    {:noreply, socket}
  end
...
SQLite is **ideal** for ExMUD because:
...
config :exmud, ExMUD.Repo,
  database: Path.expand("../data/exmud.db", __DIR__),
  pool_size: 5,
...
if config_env() == :prod do
  config :exmud, ExMUD.Repo,
    database: "/data/exmud.db",  # Fly volume mount point
...
BACKUP_FILE="/data/backups/exmud_${DATE}.db"

# SQLite backup while database is running (uses WAL)
sqlite3 /data/exmud.db ".backup '${BACKUP_FILE}'"
...
| Feature | SQLite | PostgreSQL | ExMUD Needs |
|---------|--------|------------|-------------|
...
For ExMUD's scale (starting out), SQLite is **actually better** because:
...
defmodule ExMUD.Accounts.Player do
  ...
  schema "players" do
    ...
    # Relationships
    has_many :characters, ExMUD.Game.Character
    has_one :wallet, ExMUD.Economy.Wallet
    has_many :player_tokens, ExMUD.Accounts.PlayerToken
  ...
  defp validate_username(changeset) do
    ...
    |> unsafe_validate_unique(:username, ExMUD.Repo)
...
```elixir
# lib/exmud_web/plugs/api_auth_plug.ex
defmodule ExMUDWeb.Plugs.APIAuth do
  @moduledoc """
...
defmodule ExMUDWeb.Plugs.ChannelAuth do
  @moduledoc """
...
```elixir
# lib/exmud/debug/logger.ex
defmodule ExMUD.Debug.Logger do
  @moduledoc """
  ...
  def log(%{level: level, msg: {:string, msg}, meta: meta}, _config) do
    if should_broadcast?() do
      ...
      Phoenix.PubSub.broadcast(ExMUD.PubSub, "debug:logs", {:log, log_entry})
    end
  end
  ...
  defp should_broadcast? do
    Application.get_env(:exmud, :debug_logging, false) &&
...
```elixir
# lib/exmud_web/channels/debug_channel.ex
defmodule ExMUDWeb.DebugChannel do
  use Phoenix.Channel
  ...
  defp authorized?(token) do
    case Mix.env() do
      ...
      :staging -> token == Application.get_env(:exmud, :debug_token)
...
```elixir
# lib/exmud_web/live/debug/logs_live.ex
defmodule ExMUDWeb.Debug.LogsLive do
  use ExMUDWeb, :live_view
  ...
  def mount(_params, session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(ExMUD.PubSub, "debug:logs")
    end
...
```elixir
# lib/exmud/game/session.ex

defmodule ExMUD.Game.Session do
  require Logger
  alias ExMUD.Debug.Logger, as: DebugLog
  ...
  def process_command(session, raw_input) do
    ...
    case ExMUD.Command.Parser.parse(raw_input, session.command_sets) do
...
echo "🔧 ExMUD Debug Tools"
echo "===================="
echo ""
echo "1. View live logs:     https://exmud.fly.dev/debug/logs"
echo "2. Stream server logs: fly logs -a exmud"
echo "3. SSH into server:    fly ssh console -a exmud"
echo ""
echo "Quick commands:"
echo "  fly logs -a exmud                    # Stream all logs"
echo "  fly logs -a exmud | grep ERROR       # Only errors"
echo "  fly logs -a exmud | grep GameEvent   # Game events only"
echo ""
...
case "$1" in
  logs)
    ...
    ;;
  errors)
    fly logs -a exmud | grep -E "(ERROR|error|Error)"
    ;;
  ssh)
...
defmodule ExMUD.API.MCP do
  ...
  pipeline :mcp do
    plug :accepts, ["json"]
    plug ExMUD.API.AuthPlug
    plug ExMUD.API.RateLimitPlug
  end
  
  scope "/api/mcp", ExMUD.API.MCP do
    pipe_through :mcp
...
defmodule ExMUD.API.MCP.GenerateController do
...
```elixir
# config/runtime.exs
config :exmud, ExMUD.Cluster,
  # Use libcluster for automatic node discovery
  topologies: [
    exmud: [
      strategy: Cluster.Strategy.Kubernetes,
      config: [
        kubernetes_selector: "app=exmud",
        kubernetes_node_basename: "exmud"
      ]
    ]
  ]

# Horde for distributed process registry
config :exmud, :distributed,
  entity_registry: ExMUD.Horde.EntityRegistry,
  entity_supervisor: ExMUD.Horde.EntitySupervisor
```

### 14.2 Single Node vs Cluster Mode

```elixir
defmodule ExMUD.Distribution do
  @moduledoc """
  ...
  def entity_registry do
    if clustered?() do
      ExMUD.Horde.EntityRegistry
    else
      ExMUD.Local.EntityRegistry
    end
  end
  ...
  def start_entity(entity_id) do
    if clustered?() do
      Horde.DynamicSupervisor.start_child(
        ExMUD.Horde.EntitySupervisor,
        {ExMUD.EntityServer, entity_id}
      )
    else
      DynamicSupervisor.start_child(
        ExMUD.Local.EntitySupervisor,
        {ExMUD.EntityServer, entity_id}
      )
    end
  end
  ...
  defp clustered? do
    Application.get_env(:exmud, :distributed, false)
  end
end
```
...
defmodule ExMUD.EntityTest do
  ...
  describe "entity creation" do
    test "creates entity with components" do
      entity = ExMUD.Builder.create_entity(:npc, %{
        name: "Test NPC",
        components: [
      ...
      assert ExMUD.Entity.has_component?(entity, Combatant)
      assert ExMUD.Entity.get_component(entity, Combatant).health.max == 100
    end
  end
  ...
  describe "behavior processing" do
    test "handles look event" do
      ...
      {:ok, _entity, events} = ExMUD.Behaviors.DefaultRoom.handle_event(room, event, %{})
...
```elixir
defmodule ExMUD.ScriptingTest do
  use ExUnit.Case
  ...
  describe "sandbox security" do
    test "blocks file system access" do
      ...
      assert {:error, _} = ExMUD.Scripting.execute(script, %{})
    end
    ...
    test "enforces CPU limits" do
      ...
      assert {:error, :reduction_limit_exceeded} = 
        ExMUD.Scripting.execute(script, %{})
    end
    ...
    test "allows game API calls" do
      ...
      assert {:ok, _} = ExMUD.Scripting.execute(script, mock_entity(), %{player: mock_player()})
    end
  end
end
```
...
const PRODUCT_IDS = {
  ...
  subscriptions: [
    'com.exmud.sub.basic.monthly',
    'com.exmud.sub.premium.monthly',
  ],
};
  ...
  preview:
    ...
    steps:
      ...
      - name: Configure environment
        run: |
          echo "API_URL=https://exmud.fly.dev" >> .env.preview
          echo "SOCKET_URL=wss://exmud.fly.dev/socket" >> .env.preview
...
COPY --from=build --chown=nobody:root /app/_build/${MIX_ENV}/rel/exmud ./
...

{
  ...
  "build": {
    ...
    "preview": {
      ...
      "env": {
        "API_URL": "https://exmud.fly.dev",
        "SOCKET_URL": "wss://exmud.fly.dev/socket"
      },
      "channel": "preview"
    },
    "production": {
...

{
  "expo": {
    ...
    "ios": {
      "supportsTablet": true,
      "bundleIdentifier": "com.exmud.app",
      "buildNumber": "1",
      "infoPlist": {
...
echo "🚀 Setting up ExMUD development environment..."
...
mkdir -p exmud/{apps/{server,mobile},packages,docs,scripts,.github/workflows}
cd exmud
git init
...
# Note: No DATABASE_URL needed! SQLite uses local file at /data/exmud.db
```
...
open https://exmud.fly.dev/debug/logs     # Debug dashboard

# Database (SQLite)
fly ssh console -a exmud                  # SSH into server
/app/bin/exmud remote                     # IEx console
