...
ExMUD uses a lazy-loading pattern where entities only become active GenServer processes when accessed. This reduces memory usage at scale while maintaining responsive gameplay.
...
children = [
  {DynamicSupervisor, name: Exmud.Engine.EntitySupervisor, strategy: :one_for_one}
]
```
...
Exmud.Application
       ...
       ├── Exmud.Engine.EntitySupervisor (DynamicSupervisor)
       │         │
