...
| Observability design | 1 | between R4 and R5 ([owner decision](decisions/owner-decisions-observability-astra-2026-09-25.md)): one record format and event-name registry validated in CI, the game-trace format (derived from committed state, never authority), shared correlation ids, explicit unknown/unavailable states, redaction; stores by purpose, joined by ids. Built so `loka play` and the simulator emit it from R5's first slice; export and cross-store search later. Spec 11 §11-15, 08 §6 |
