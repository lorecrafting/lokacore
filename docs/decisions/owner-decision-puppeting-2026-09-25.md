...
`world.body`; the only single-controller assumption is at the admission boundary and the
`loka play` host. Add one contract lesson now so no rule hard-codes the player (rules read
