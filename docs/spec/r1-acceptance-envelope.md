...
**Version:** 0.6 — original numerical criteria retained; owner-approved staged preparation and delegated hardware selection, 2026-09-23 (ADR-064); proposed A1-only hosted execution host, 2026-09-23 (§4); proposed candidate order C, B, A with randomized differential testing, 2026-09-23 (§2, §3; ADR-068). Amendment review/merge, R0, setup reviews and measurements remain pending. **No candidate has been selected, benchmarked, or accepted.** Freeze accepted envelope bytes, reviewed fixtures and the reproducible A1 execution setup before semantic implementation. Freeze complete native/physical setup and obtain new stage-bound review before A2; preparation may proceed now. Amendments after measurement must identify the earlier measurements and reason; never tune the acceptance rule to rescue a favorite candidate.
...
C is tested first, so the online server stays a single Elixir/OTP runtime (owner direction, 2026-09-23; ADR-068). Passing all applicable MUST gates selects the simplest sufficient C design; A/B need not be built. This is a sufficiency procedure, **not** a claim that unmeasured alternatives are inferior. If C fails, record results and evaluate B; if B fails, evaluate A. R1-A1 through R1-A5 name work stages, not candidate A. Infeasible/unavailable measurements remain unmeasured and cannot pass. Any new boundary variant is recorded before its performance tuning; all common safety and load gates still apply.
...
Record process RSS, attributable live/retained heap, shared runtime baseline, per-instance cache, and disk growth over 10,000 inputs after warm-up. Report the whole server runtime baseline, the BEAM for C and also A's Node runner or B's isolated worker where present, as well as incremental hosted-instance cost; do not divide away fixed overhead.
...
| Isolated runner death (A, or B's isolated worker; not applicable to C) | Detect and reconstruct from committed state; proposed state is not authoritative; approved target Tiny worker recovery target <=1 second |
...
For every applicable row: threshold, exact input/build/device identity, observed value, sample count, pass/fail/unmeasured, and evidence location. Hashes supplement retained canonical bytes; unknowns are not passes. Compare maintenance and fault containment as well as speed. Record why untested candidates were skipped under the C-first sufficiency rule (ADR-068).
