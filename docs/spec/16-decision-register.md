...
**Status:** Accepted by the owner, 2026-09-25 ([record](../decisions/owner-decision-adr-075-2026-09-25.md)); full text in [ADR-075](../decisions/adr-075-observability-proposal.md). Amended by R5 slice 1 (PR #34, [review](../reviews/2026-09-25-r5-s1-review.md)): the `trace.run` header carries `world_context_id`; replay runs under the recorded `kernel_version` and reports, never compares, it; free player text enters a Command only where its contract needs it, after redaction or rejection. Amended by R5 slice 2 (owner request): a failed lookup writes one `target.unresolved` diagnostics record whose player words are redacted before it is built.
