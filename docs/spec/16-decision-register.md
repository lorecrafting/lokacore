...
Every log, trace, metric and dev-evidence record is an `ObservationRecord` (`protocol/observation.schema.json`, format `loka-obs-v1`) in canonical JSON Lines, named in `protocol/event_registry.json`. Four stores (`game_trace`, `diagnostics`, `operations`, `dev_evidence`) are joined by closed, per-store correlation ids; `kernel_version` is the source revision. Unknown and unavailable measures are explicit, never 0 or absent. Observation explains and never decides: a fix is proven by replaying the seed. A name is registered with its first producer, and every producer validates its records in CI.
