...
*Amendment 2026-09-25 (R5 S4, [owner decisions](../decisions/owner-decisions-r5-s4-2026-09-25.md)):* items and NPCs take the frozen shapes of `protocol/entity.schema.json`, which supersede this example's item and NPC files: `items/<key>.json` carries `keywords` (what the player types), `short` ("a brass lantern", for action messages and the inventory), `room_line` ("A brass lantern lies in the weeds.", with optional `room_line_variants` chosen as description variants are), `description` (for examine), `location` as `{"in": "room", "room": "village_green"}` (or `npc` or `item`, the containers an item can start in) and an optional `capacity`; `npcs/<key>.json` carries the same text fields, its `room` and an optional `capacity` (dialogue, schedules and quest giving join with their capabilities). Items and NPCs require `containment@1`, which owns take, drop, give and `has_item`. A catalog string marks its tappable words inline as touch links: `[brass lantern]` in an item's own text links that item, and the words followed by a key in parentheses link that detail of the room, item or NPC; an unresolved target fails the build and a missing link is a warning (`protocol/cartridge.schema.json` DiagnosticCode, TOUCH_LINK_MISSING). This example's `take` check (`kind: luck`) waits for checks (R5 S6).
