...
- [R5 S6b: resources (HP/MA/MV), costs, threshold checks, cooldowns, 1 MV per move, regeneration](2026-09-25-r5-s6b-review.md): PR #49 at `7cb4af2`, APPROVE WITH NOTES (two should-fix: an over-maximum `resource.adjust` step faults in ordinary play with no author guard; a long cooldown overflows into `evaluator_error`), plus Astra REQUEST CHANGES (A1 MV dead end without `wait`; A2 GameView availability ignores cooldown, costs and MV); fixes `be01d41`, APPROVE (one nit: saturation at the minimum untested); fix `b8b97af`, APPROVE.
- [Playtest-and-tune owner decision; every cartridge gets the pools](2026-09-25-pm-docs-playtest-review.md): PR #48 at `70636c5`, APPROVE WITH NOTES (one nit: the ROADMAP row's "on the phone" is not in the record).
- [R5 S7: barrier@1 (doors, locks, keys, `barrier.transition` in both kernels) and room brief mode](2026-09-26-r5-s7-review.md): PR #50 at `2d5c6fb`, APPROVE WITH NOTES (three nits: TS compose throws where Elixir faults on a prototype-named barrier state; BARRIER_MISMATCH pairs every back exit, not the reciprocal face; keyless lock wording; two questions: container locks and the direction-only command, touch `close`), plus Sol 5.6 REQUEST CHANGES (A1 blocker: no barrier/key lockout check; A2 faces by destination; A3 compose fallback); fixes `c5afed4`, CHANGES REQUIRED (one blocker: compiler container reach untested; two should-fix: stale Connection face-rule text, face "leads back" condition untested). Fix `dbfab20`, APPROVE.
- [#47-#50 ledger, S8 and R5 carries, GameView need 12, cross-vendor wording](2026-09-26-pm-docs-s7-ledger-review.md): PR #51 at `a0114eb`, APPROVE WITH NOTES (two nits: WORKFLOW still says "Astra" at two cross-vendor spots; locked containers carried out of R5 while 00 still lists them at R5; two questions: the source of S8 `scan`, the early R7/R8 count); fixes `e7811c8`, APPROVE.
- [R5 S8: `scan` (movement@1), loader touch-link parity, `implemented_in` held to the code, mobile require/jest lint](2026-09-26-r5-s8-review.md): PR #52 at `870cca1`, APPROVE WITH NOTES (one nit: the jest list misses `setMock` and `createMockFromModule`); fix `8476abf`, APPROVE.
- [R5 +1: deterministic simulation (seeded sequences, per-step invariants, GameView agreement, shrinking, 10k fresh in CI)](2026-09-26-r5-sim-review.md): PR #53 at `da87c36`, CHANGES REQUIRED (one blocker: the recipe half and the reason-code clause of `gameview_agrees_with_admission` survive deletion; one should-fix: the op/event half of `unknown_types_fail_closed` has no red control; two nits: `REACHED` exact list, transcript stops at a stray command).
