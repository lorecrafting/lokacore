...
The admin dashboard provides world-building tools and system management for ExMUD.
...
| File | Purpose |
|------|---------|
| `lib/exmud_web/live/admin_live.ex` | Main LiveView |
| `lib/exmud_web/plugs/require_admin.ex` | Access control |
| `lib/exmud/accounts.ex` | Player management |
| `lib/exmud/engine/entities.ex` | Entity CRUD |
| `lib/exmud/engine/scripts.ex` | Script CRUD |

## Making a User Admin

Via IEx console:
```elixir
player = Exmud.Accounts.get_player_by_email("user@example.com")
Exmud.Accounts.set_admin(player, true)
```
