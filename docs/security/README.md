# ExMUD Security Documentation

This document covers the security measures implemented in ExMUD to protect against common vulnerabilities and attacks.
...
ExMUD uses Phoenix 1.8's magic link authentication by default:
...
See `lib/exmud_web/plugs/rate_limiter.ex`
...
1. **Map key atomization** uses `String.to_existing_atom/1` only:
   ...
   ```elixir
   # lib/exmud/utils/map_helpers.ex
...
2. **Dialogue actions** use a whitelist:
   ...
   ```elixir
   # lib/exmud/framework/dialogue/dialogue.ex
...
See `lib/exmud/engine/scripting.ex` for implementation.
