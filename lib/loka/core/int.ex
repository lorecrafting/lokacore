defmodule Loka.Core.Int do
  ...
  Operands and results stay in [-(2^53 - 1), 2^53 - 1]; leaving it is `:integer_overflow`,
  never wraparound. BEAM integers are exact, so results are checked after computing.
  """
  ...
  @spec add(integer(), integer()) :: {:ok, integer()} | {:error, :integer_overflow}
  def add(a, b) when safe(a) and safe(b), do: check(a + b)

  @spec sub(integer(), integer()) :: {:ok, integer()} | {:error, :integer_overflow}
  def sub(a, b) when safe(a) and safe(b), do: check(a - b)

  @spec mul(integer(), integer()) :: {:ok, integer()} | {:error, :integer_overflow}
  ...
  @spec divide(integer(), integer()) :: {:ok, integer(), integer()} | {:error, :division_by_zero}
  def divide(_a, 0), do: {:error, :division_by_zero}
  # div/rem truncate toward zero; the range is symmetric, so no quotient overflows.
  def divide(a, b) when safe(a) and safe(b), do: {:ok, div(a, b), rem(a, b)}
